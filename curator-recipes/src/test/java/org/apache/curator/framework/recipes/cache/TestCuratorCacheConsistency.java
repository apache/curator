...
public class TestCuratorCacheConsistency extends CuratorTestBase {
    ...
    private class Client implements Closeable {
        ...
        Client(int index, String connectionString, AtomicReference<Exception> errorSignal) {
            ...
            cache = CuratorCache.builder(client, BASE_PATH)
                    .withOptions(DO_NOT_CLEAR_ON_CLOSE)
                    .withExceptionHandler(errorSignal::set)
                    .build();
            ...
            CuratorCacheListener listener = builder()
                    .forCreates(node -> {
                        ...
                        if (previous != null) {
                            errorSignal.set(new Exception(
                                    String.format("Client: %d - Create for existing node: %s", index, node.getPath())));
                        }
                    })
                    .forChanges((oldNode, node) -> {
                        ChildData previous = listenerDataMap.put(node.getPath(), node);
                        if ((previous == null) || !Arrays.equals(previous.getData(), oldNode.getData())) {
                            errorSignal.set(new Exception(String.format(
                                    "Client: %d - Bad old value for change node: %s", index, node.getPath())));
                        }
                    })
                    .forDeletes(node -> {
                        ...
                        if (previous == null) {
                            errorSignal.set(new Exception(String.format(
                                    "Client: %d - Delete for non-existent node: %s", index, node.getPath())));
                        }
                    })
                    .build();
    ...
    private void buildActual(CuratorFramework client, Map<String, String> actual, String fromPath) {
        try {
            ...
            client.getChildren()
                    .forPath(fromPath)
                    .forEach(child -> buildActual(client, actual, ZKPaths.makePath(fromPath, child)));
        } catch (Exception e) {
            fail("", e);
        }
    }
