...
public class ModeledFrameworkImpl<T> implements ModeledFramework<T> {
    ...
    public static <T> ModeledFrameworkImpl<T> build(
        ...
        AsyncCuratorFrameworkDsl dslClient =
                client.with(watchMode, unhandledErrorListener, resultFilter, watcherFilter);
    ...
    private ModeledFrameworkImpl(
            AsyncCuratorFramework client,
            AsyncCuratorFrameworkDsl dslClient,
            WatchableAsyncCuratorFramework watchableClient,
    ...
    public CachedModeledFramework<T> cached(ExecutorService executor) {
        Preconditions.checkState(
                !isWatched,
                "CachedModeledFramework cannot be used with watched instances as the internal cache would bypass the watchers.");
    ...
    public AsyncStage<Void> delete(int version) {
        return dslClient
                .delete()
                .withOptionsAndVersion(modelSpec.deleteOptions(), version)
    ...
    private AsyncStage<List<ZPath>> internalGetChildren(ZPath path) {
        ...
        asyncStage.whenComplete((children, e) -> {
            if (e != null) {
                if (modeledOptions.contains(ModeledOptions.ignoreMissingNodesForChildren)
                        && (Throwables.getRootCause(e) instanceof KeeperException.NoNodeException)) {
    ...
    private <U> ModelStage<U> internalRead(Function<ZNode<T>, U> resolver, Stat storingStatIn) {
        ...
        asyncStage.whenComplete((value, e) -> {
            if (e != null) {
                ...
                try {
                    ZNode<T> node = new ZNodeImpl<>(
                            modelSpec.path(), stat, modelSpec.serializer().deserialize(value));
