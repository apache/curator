...
public class WatcherRemovalManager
{
    ...
    private final Set<NamespaceWatcher> entries = Sets.newHashSet();  // guarded by sync
    ...
    synchronized void add(NamespaceWatcher watcher)
    {
    ...
    void removeWatchers()
    {
        Set<NamespaceWatcher> localEntries;
        synchronized(this)
        {
            localEntries = Sets.newHashSet(entries);
            entries.clear();
        }
        for ( NamespaceWatcher watcher : localEntries )
        {
            try
            {
    ...
    synchronized void noteTriggeredWatcher(NamespaceWatcher watcher)
    {
